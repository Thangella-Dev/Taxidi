...
export default function AdminDashboard() {
  ...
  const [riderSelfieUrls, setRiderSelfieUrls] = useState<
  ...
  const loadHealth = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok)
        setMessage(
          "System health is degraded. Check the Health section for deployment configuration.",
        );
    } catch {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === "DELETE") {
            ...
            if (deleted.id)
              setRiderVehicles((current) =>
                current.filter((vehicle) => vehicle.id !== deleted.id),
              );
            return;
          }
          setRiderVehicles((current) =>
            upsertById(current, payload.new as RiderVehicle),
          );
        },
      )
      .on(
          ...
          if (payload.eventType === "DELETE") {
            ...
            if (deleted.id)
              setSupportTickets((current) =>
                current.filter((ticket) => ticket.id !== deleted.id),
              );
            return;
          }
          setSupportTickets((current) =>
            sortByCreated(upsertById(current, payload.new as SupportTicket)),
          );
        },
      )
      .subscribe((status) => {
    ...
    const { error } = await supabase
      ...
      .update({
        identity_rejection_reason:
          status === "rejected"
            ? "Live identity photo or licence requires correction."
            : null,
  ...
  async function updateVehicleVerification(
    ...
    const { error } = await supabase
      ...
      .update({
        rejection_reason:
          status === "rejected"
            ? "Vehicle details require correction or manual review."
            : null,
        verification_status: status,
        updated_at: new Date().toISOString(),
      })
  ...
  const dashboardStats = useMemo(
    () => ({
      activeRides: rides.filter(
        (ride) => ride.status === "assigned" || ride.status === "started",
      ).length,
      ...
      verificationQueue:
        riderProfiles.filter((rider) => rider.verification_status === "pending")
          .length +
        riderVehicles.filter(
  ...
  return (
    <AppShell title="Admin dashboard">
      <div className="min-w-0 space-y-5 pb-8" id="admin-overview">
        {message ? (
          <div
            className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900 shadow-sm"
            role="status"
          >
            {message}
          </div>
          ...
          <div className="grid gap-6 p-5 sm:p-7 xl:grid-cols-[minmax(0,1.1fr)_minmax(22rem,0.9fr)] xl:items-end">
            <div className="min-w-0">
              <div className="mb-5 flex flex-wrap items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-lime-200/80">
                ...
                <span className="rounded-full bg-lime-300 px-3 py-1 text-[11px] text-[#07110d]">
                  Live operations
                </span>
              </div>
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">
                Monitor rides, verify riders, send alerts, control account
                access, and watch platform health from one clean operations
                surface.
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                ...
                <HeroSignal
                  label="Ready signals"
                  value={dashboardStats.readyRides}
                  tone="white"
                />
                <HeroSignal
                  label="Verification queue"
                  value={dashboardStats.verificationQueue}
                  tone="amber"
                />
              </div>
            </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-3xl font-black">
                    {formatMoney(moneyStats.gross)}
                  </p>
                </div>
                <span className="grid size-12 place-items-center rounded-2xl bg-lime-300 text-[#07110d]">
                  <IndianRupee className="size-6" />
                </span>
              </div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-2xl bg-white/[0.07] p-3">
                  <p className="text-white/55">Taxiro share</p>
                  <p className="text-xl font-black">
                    {formatMoney(moneyStats.company)}
                  </p>
                </div>
                <div className="rounded-2xl bg-white/[0.07] p-3">
                  <p className="text-white/55">Rider earnings</p>
                  <p className="text-xl font-black">
                    {formatMoney(moneyStats.riders)}
                  </p>
                </div>
              </div>
              <p className="text-xs font-semibold text-white/50">
                Realtime rows are merged live and resynced every few seconds.
              </p>
            </div>
          </div>
        </section>
        ...
        <nav
          ...
          className="sticky top-16 z-30 flex gap-2 overflow-x-auto rounded-2xl border border-border bg-background/92 p-2 shadow-sm backdrop-blur [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
        ...
        {activeSection === "verification" ? (
          <RiderVerificationPanel
            onUpdateIdentity={updateIdentityVerification}
            onUpdateVehicle={updateVehicleVerification}
            profiles={profiles}
            riderProfiles={riderProfiles}
            riderSelfieUrls={riderSelfieUrls}
        ...
        {activeSection === "controls" ? (
          <AdminOperationalControls onMessage={setMessage} />
        ) : null}
        {activeSection === "rides" ? (
          <RideOperationsPanel
...
function SystemHealthPanel({
  ...
  return (
    <section
      className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(22rem,0.72fr)]"
      id="admin-health"
    >
      <Card className="rounded-[1.75rem] p-5">
        <CardHeader className="mb-4 flex flex-row items-start justify-between gap-4 p-0">
          <div>
            ...
            <CardDescription className="mt-2 max-w-2xl">
              Track production readiness, environment configuration, and
              deployment action items from inside the admin panel.
            </CardDescription>
          </div>
        ...
        {health ? (
          <div className="grid gap-3 sm:grid-cols-2">
            {checks.map(([key, check]) => {
              const label = check.ok
                ? check.status === "access_restricted"
                  ? "Restricted"
                  : "OK"
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`shrink-0 rounded-full px-3 py-1 text-xs font-black ${tone}`}
                    >
                      {label}
                    </span>
                  </div>
                  {check.migration ? (
                    <p className="mt-3 rounded-xl bg-background px-3 py-2 text-xs font-black text-muted-foreground">
          ...
          <div className="rounded-2xl bg-muted p-5 text-sm font-semibold text-muted-foreground">
            Health data has not loaded yet. Use Refresh to check /api/health.
          </div>
        )}
      </Card>
      ...
      <div className="grid gap-5">
        <Card className="rounded-[1.75rem] bg-[#07110d] p-5 text-white">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black">
                {health?.service ?? "taxiro-web"}
              </h2>
            </div>
            <span
              className={`rounded-full px-3 py-1 text-xs font-black uppercase ${statusTone}`}
            >
          ...
          <div className="mt-5 grid gap-3 text-sm">
            ...
            <DeploymentFact
              label="Environment"
              value={health?.deployment.environment ?? "local"}
            />
            <DeploymentFact
              label="Region"
              value={health?.deployment.region ?? "local"}
            />
            <DeploymentFact
              label="URL"
              value={health?.deployment.url ?? "local"}
            />
            <DeploymentFact
              label="Checked"
              value={
        ...
        <Card className="rounded-[1.75rem] p-5">
          <CardHeader className="mb-3 p-0">
            ...
            <CardDescription>
              Use this when Vercel or Supabase checks fail after a push.
            </CardDescription>
          </CardHeader>
          <div className="grid gap-2">
            {(
              health?.recommendations ?? [
                "Refresh health to load deployment action items.",
              ]
            ).map((item) => (
              <p
                className="rounded-2xl bg-muted p-3 text-sm font-semibold text-muted-foreground"
                key={item}
              >
                {item}
              </p>
...
function DeploymentFact({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-2xl bg-white/[0.07] p-3">
...
function HeroSignal({
  ...
  label: string;
  tone: "amber" | "lime" | "white";
  value: number;
}) {
  const toneClass = {
    amber: "bg-amber-300 text-[#07110d]",
    lime: "bg-lime-300 text-[#07110d]",
...
function MetricTile({
  ...
  icon: LucideIcon;
  label: string;
  tone?: "amber" | "default" | "green" | "lime" | "red";
  value: number | string;
}) {
  const toneClass = {
  ...
  return (
    <Card className="group rounded-2xl p-4 transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="rounded-full bg-muted px-2.5 py-1 text-[11px] font-black uppercase text-muted-foreground">
          Live
        </span>
      </div>
    ...
    <Card className="rounded-[1.75rem] bg-[#f4f7f1] p-5 xl:sticky xl:top-36">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-black">Platform snapshot</h2>
        </div>
      ...
      <div className="mt-5 grid gap-3">
        <SnapshotRow
          icon={Activity}
          label="Active ride load"
          value={`${activeRides} trips`}
        />
        <SnapshotRow
          icon={Megaphone}
          label="Ready demand"
          value={`${readyRides} signals`}
        />
        <SnapshotRow
          icon={UserCheck}
          label="Online supply"
          value={`${onlineRiders} riders`}
        />
        <SnapshotRow
          icon={ShieldCheck}
          label="Verification queue"
      ...
      <div className="mt-5 rounded-2xl bg-card p-4">
        <p className="text-sm font-black">Latest ride</p>
        {latestRide ? (
          <div className="mt-3 space-y-2 text-sm">
            <p className="font-black">Ride #{latestRide.id.slice(0, 8)}</p>
            ...
            <div className="flex flex-wrap gap-2 text-xs font-bold">
              ...
              <span className="rounded-full bg-muted px-2 py-1">
                {getVehicleLabel(latestRide.vehicle_type)}
              </span>
              <span className="rounded-full bg-muted px-2 py-1">
                {formatMoney(latestRide.fare_estimate ?? 0)}
              </span>
            </div>
          </div>
        ) : (
...
function RiderVerificationPanel({
  onUpdateIdentity,
  onUpdateVehicle,
  profiles,
  riderProfiles,
  ...
  riderSelfieUrls: Record<string, string>;
  riderVehicles: RiderVehicle[];
}) {
  const pendingIdentity = riderProfiles.filter(
    (rider) => rider.verification_status !== "verified",
  );
  const pendingVehicles = riderVehicles.filter(
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
              Approve live identity first, then verify each Bike, Auto, or Car.
              Photos are shown as compact review thumbnails so the queue stays
              scannable.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:min-w-[28rem]">
            <VerificationStat
              ...
              value={pendingIdentity.length}
              tone="amber"
            />
            <VerificationStat
              label="Identity verified"
              value={verifiedIdentityCount}
              tone="green"
            />
            <VerificationStat
          ...
          <div className="mb-4 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="flex items-center gap-2 text-sm font-black">
                <Camera className="size-4" /> Identity review
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                Live selfie and licence context
              </p>
            </div>
            <span className="shrink-0 rounded-full bg-muted px-3 py-1 text-xs font-black">
              {pendingIdentity.length} pending
            </span>
          </div>
            ...
            {riderProfiles.length ? (
              riderProfiles.map((rider) => {
                const person = profiles.find(
                  (item) => item.id === rider.rider_id,
                );
                const verified = rider.verification_status === "verified";
                const photoUrl = riderSelfieUrls[rider.rider_id];
                return (
                  <article
                    className="rounded-2xl border border-border bg-muted/55 p-3 transition hover:bg-muted/80"
                    key={rider.rider_id}
                  >
                    <div className="grid gap-3 sm:grid-cols-[8rem_minmax(0,1fr)]">
                      <div className="overflow-hidden rounded-2xl bg-card shadow-sm">
                        {photoUrl ? (
                          <Image
                            alt={
                              "Live identity capture for " +
                              (person?.full_name ?? "rider")
                            }
                          ...
                          <div className="grid h-36 place-items-center p-3 text-center text-xs font-semibold text-amber-800">
                            No live photo submitted
                          </div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="truncate text-lg font-black">
                              {person?.full_name ?? rider.rider_id.slice(0, 8)}
                            </p>
                            <p className="mt-1 truncate text-xs font-semibold text-muted-foreground">
                              {person?.phone ?? "No phone on profile"}
                            </p>
                          </div>
                        ...
                        <div className="mt-3 grid gap-2 rounded-xl bg-card p-3 text-xs">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="truncate font-black">
                              {rider.license_number ?? "Not submitted"}
                            </span>
                          </div>
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-bold text-muted-foreground">
                            ...
                            <span className="truncate font-black capitalize">
                              {rider.active_vehicle_type ?? "Not selected"}
                            </span>
                          </div>
                        </div>
                        {rider.identity_rejection_reason ? (
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <Button
                            disabled={!rider.live_selfie_path || verified}
                            onClick={() =>
                              void onUpdateIdentity(rider.rider_id, "verified")
                            }
                            size="sm"
                          >
                          ...
                          <Button
                            onClick={() =>
                              void onUpdateIdentity(rider.rider_id, "rejected")
                            }
                            size="sm"
                            variant="outline"
                          >
          ...
          <div className="grid max-h-[min(70dvh,42rem)] gap-3 overflow-y-auto pr-1">
            {riderVehicles.length ? (
              riderVehicles.map((vehicle) => {
                const rider = riderProfiles.find(
                  (item) => item.rider_id === vehicle.rider_id,
                );
                const person = profiles.find(
                  (item) => item.id === vehicle.rider_id,
                );
                const verified = vehicle.verification_status === "verified";
                const canVerify = Boolean(
                  rider?.live_selfie_path &&
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-lg font-black capitalize">
                    ...
                    <div className="mt-3 grid gap-2 rounded-xl bg-card p-3 text-xs sm:grid-cols-2">
                      <div>
                        <p className="font-bold uppercase tracking-[0.12em] text-muted-foreground">
                          Registration
                        </p>
                      ...
                      <div>
                        ...
                        <p className="mt-1 truncate text-base font-black">
                          {person?.full_name ?? vehicle.rider_id.slice(0, 8)}
                        </p>
                      </div>
                    </div>
                    {rider?.active_vehicle_type === vehicle.vehicle_type ? (
                      <p className="mt-2 rounded-xl bg-lime-100 px-3 py-2 text-xs font-black text-lime-800">
                        Currently active for matching
                      </p>
                    ) : null}
                    {!canVerify ? (
                      <p className="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
                        Identity must be approved before this vehicle can be
                        verified.
                      </p>
                    ) : null}
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <Button
                        ...
                        onClick={() =>
                          void onUpdateVehicle(vehicle.id, "verified")
                        }
                        size="sm"
                      >
                      ...
                      <Button
                        onClick={() =>
                          void onUpdateVehicle(vehicle.id, "rejected")
                        }
                        size="sm"
                        variant="outline"
                      >
...
function ReviewBadge({
  ...
  return (
    <span
      className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black uppercase ${classes}`}
    >
      {status}
    </span>
  );
...
function RideOperationsPanel({
  ...
  filteredRides: RideRequest[];
  query: string;
  setQuery: (value: string) => void;
  setStatusFilter: (value: "all" | RideRequest["status"]) => void;
  statusFilter: "all" | RideRequest["status"];
}) {
  return (
      ...
      <Card className="rounded-[1.5rem] p-4 sm:p-5">
        <div className="mb-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black">
              Dispatch and ride audit
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Search live rides by passenger, pickup, drop, vehicle, or ride ID.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-[minmax(16rem,1fr)_auto]">
            ...
            <select
              className="h-11 rounded-full border border-border bg-card px-4 text-sm font-black outline-none focus:ring-2 focus:ring-ring"
              onChange={(event) =>
                setStatusFilter(
