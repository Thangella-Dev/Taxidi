...
export default function RiderDashboard() {
  ...
  const [riderHomeView, setRiderHomeView] = useState<
    "ready" | "advance" | "route"
  >("ready");
  ...
  const changeAvailability = useCallback(
    async (available: boolean) => {
      ...
      setMessage(
        available
          ? "You are online and visible for matching."
          : "You are offline for this app session.",
      );
      return null;
    },
    [persistAvailability],
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const visibleToRider =
            ["scheduled", "ready"].includes(incoming.status) ||
            incoming.assigned_rider_id === riderId;
          setRides((current) => {
            if (!visibleToRider)
              return current.filter((ride) => ride.id !== incoming.id);
          ...
          if (incoming.rider_id === riderId) {
            ...
            setGpsStatus(
              incoming.last_seen_at
                ? `Live GPS ${formatTrackingAge(incoming.last_seen_at)}`
                : "Location updated.",
            );
          }
        },
      ...
      .on(
        ...
        (payload) => {
          ...
          if (incoming.rider_id === riderId)
            setRiderVehicles((current) => upsertById(current, incoming));
        },
      )
      .on(
        "postgres_changes",
      ...
      .subscribe((status) => {
        if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
          setMessage(
            "Live rider updates are reconnecting. Keep Taxiro open for instant jobs and tracking.",
          );
        }
      });
  ...
  useEffect(() => {
    ...
    const stop = watchRiderLocation({
      ...
      onUpdate: (liveLocation: RiderTrackingUpdate) => {
        ...
        setGpsStatus(
          `Live GPS on${liveLocation.accuracy_m ? ` - +/-${Math.round(liveLocation.accuracy_m)}m` : ""}`,
        );
      },
      riderId: profile.id,
      supabase,
  ...
  async function updateLocation(
    point: LatLng & Partial<RiderTrackingUpdate>,
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "GPS detection failed. Choose your location on the map.";
  ...
  async function switchVehicle(vehicleType: VehicleType) {
    ...
    setMessage(
      `${vehicleType[0].toUpperCase()}${vehicleType.slice(1)} is now active for matching.`,
    );
    await loadRiderData(profile.id);
  }
  ...
  async function markReachedDrop(ride: RideRequest) {
    ...
    setMessage(
      error
        ? error.message
        : "Drop reached. Collect payment, then confirm payment received.",
    );
    await loadRiderData(profile.id);
  }
  ...
  const hasVerifiedActiveVehicle = Boolean(
    ...
    riderVehicles.some(
      (vehicle) =>
        vehicle.vehicle_type === activeVehicleType &&
  ...
  const visibleDemandRides = rides
    ...
    .sort((a, b) => {
      ...
      const distanceDelta =
        approxDistanceKm(location, a) - approxDistanceKm(location, b);
  ...
  const readyRides = visibleDemandRides.filter(
    (ride) => ride.status === "ready",
  );
  const scheduledRides = visibleDemandRides.filter(
  ...
  const mapRiders = useMemo(
    () =>
      riderLocation
        ? [{ ...riderLocation, lat: location.lat, lng: location.lng }]
        : [],
    ...
    return (
      <AppShell title="Rider app">
        <Card className="mx-auto max-w-lg text-center">
          <CardHeader>
            <CardTitle>
              {profile ? "Rider account required" : "Sign in required"}
            </CardTitle>
            <CardDescription>
              {profile
      ...
      <div className="taxiro-immersive-stage taxiro-responsive-stage relative min-w-0 w-full max-w-full overflow-x-clip bg-muted [contain:inline-size]">
        ...
        <div className="taxiro-overlay-bar pointer-events-none absolute inset-x-2 top-0 z-[1200] flex items-start justify-between gap-2 sm:inset-x-3 sm:gap-3 lg:inset-x-4">
          <div className="pointer-events-auto min-w-0 flex-1 overflow-hidden rounded-xl border border-white/80 bg-white/94 p-2.5 shadow-[var(--shadow-soft)] backdrop-blur-xl sm:max-w-sm sm:p-3">
            ...
            <p className="flex min-w-0 items-center gap-1.5 truncate text-sm font-black tracking-tight sm:gap-2 sm:text-lg">
              ...
              {activeRide
                ? riderHeadline(activeRide.status)
                : "Find nearby work"}
            </p>
          </div>
          <div className="pointer-events-auto flex min-w-0 shrink items-center gap-1 rounded-xl border border-white/80 bg-white/94 p-1 shadow-[var(--shadow-soft)] backdrop-blur-xl sm:gap-1.5">
            ...
            >
              <LocateFixed
                className={`size-4 text-primary sm:size-5 ${detectingLocation ? "animate-pulse" : ""}`}
              />
            </button>
            <button
        ...
        >
          {activeRide ? (
            ...
            <div className="grid gap-3">
              <div className="grid gap-3 sm:flex sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="text-sm leading-6 text-muted-foreground">
                    Accept ready jobs, watch advance demand, or set an
                    On-The-Way route.
                  </p>
                </div>
              ...
              {!hasVerifiedActiveVehicle ? (
                <div
                  className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-800"
                  role="status"
                >
                  {riderProfile?.verification_status !== "verified"
              ...
              <div className="grid grid-cols-3 gap-1 rounded-lg bg-muted p-1 lg:hidden">
                {(["ready", "advance", "route"] as const).map((view) => (
                  ...
                  >
                    {view === "ready"
                      ? `Ready (${readyRides.length})`
                      : view === "advance"
                        ? `Demand (${scheduledRides.length})`
                        : "Route"}
                  </button>
                ))}
              </div>
              <div className="max-h-[42svh] overflow-y-auto overflow-x-hidden pr-1 lg:hidden">
                ...
                {riderHomeView === "route" ? (
                  <div className="grid gap-3">
                    <RiderRouteIntro />
                    {profile ? (
                      <RouteSetupForm defaultExpanded riderId={profile.id} />
                    ) : null}
                  </div>
                ) : null}
              </div>
              <div className="hidden gap-3 lg:grid lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)_minmax(0,1fr)] xl:grid-cols-3">
                ...
                <RiderWorkbenchPanel
                  badge={`${scheduledRides.length} advance`}
        ...
        {menuOpen ? (
          <RiderMenu
            ...
            profile={profile}
            rides={rides.filter(
              (ride) => ride.assigned_rider_id === profile?.id,
            )}
          />
        ) : null}
      </div>
...
function isRideDemandVisibleNearRider(
  ...
  if (!activeVehicleType || ride.vehicle_type !== activeVehicleType)
    return false;
  if (!isReadyRideVisible(ride) && !isScheduledDemandVisible(ride))
    return false;
  ...
  return (
    <div className="min-w-0 rounded-lg border border-border bg-card p-3 shadow-sm">
      <div className="mb-3 flex min-w-0 items-center justify-between gap-3">
        <h2 className="truncate text-sm font-black uppercase tracking-[0.12em]">
          {title}
        </h2>
  ...
  return (
    <div className="rounded-lg bg-[#101713] p-4 text-white">
      ...
      <p className="mt-2 text-sm leading-6 text-white/65">
        Add where you are starting and where you are going. Taxiro can use this
        route to surface pickups that fit your travel direction.
      </p>
    </div>
  );
}

function ScheduledRequestCard({
  currentLocation,
  ride,
}: {
  ...
  return (
    <div className="min-w-0 rounded-lg border border-border bg-card p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Pickup is about {pickupDistance} km from your current area.
          </p>
        </div>
      ...
      <p className="mt-3 rounded-lg bg-muted p-3 text-xs leading-5 text-muted-foreground">
        This is a demand signal. It becomes acceptable when the customer taps
        I&apos;m Ready.
      </p>
    </div>
  );
...
function approxDistanceKm(from: LatLng, ride: RideRequest) {
  ...
  return (
    Math.round(earthKm * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)) * 10) /
    10
  );
}
...
function RequestCard({
  ...
  const companyCommission =
    ride.company_commission ?? fareBreakdown.companyCommission;
  const pickupDistance = approxDistanceKm(currentLocation, ride);
    ...
    <div className="min-w-0 rounded-lg border border-border bg-card p-4 shadow-sm">
      <div className="grid gap-3 sm:flex sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {pickupDistance} km away from you - {readyTimeLeft}
          </p>
        </div>
        <div className="grid grid-cols-2 gap-2 text-center sm:min-w-48">
          ...
          <div className="rounded-lg bg-secondary p-2">
            <p className="text-[10px] font-bold uppercase text-secondary-foreground/70">
              You earn
            </p>
            <p className="font-black text-secondary-foreground">
              {formatMoney(riderEarning)}
            </p>
          </div>
        </div>
      </div>
      <div className="mt-3 grid gap-2 text-sm">
        <p className="rounded-lg bg-secondary/70 px-3 py-2 text-xs font-bold">
          {ride.booking_for === "other"
            ? `Picking up ${ride.passenger_name || "another passenger"}`
            : "Customer is riding"}
          {ride.fare_rate_per_km
            ? ` - ${getVehicleLabel(ride.vehicle_type)} Rs ${(ride.fare_rate_per_km ?? 0) + (ride.vehicle_surcharge_per_km ?? 0)}/km`
            : ""}
        </p>
        <p className="flex gap-2">
          <MapPinned className="mt-0.5 size-4 shrink-0 text-primary" />
      ...
      <Button
        className="mt-4 h-12 w-full rounded-lg bg-[#101713] text-base font-black text-white hover:bg-[#101713]/90"
        onClick={onAccept}
      >
    ...
    <div className="grid gap-3">
      <div className="min-w-0">
        <Badge className="bg-primary text-primary-foreground">
          Active {getVehicleLabel(ride.vehicle_type)} job
        </Badge>
        <h1 className="mt-2 text-3xl font-black tracking-tight">
          {riderHeadline(ride.status)}
        </h1>
      ...
      <div className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-border bg-secondary/60 p-3">
        <div className="min-w-0">
          ...
          <p className="truncate font-black">
            {ride.passenger_name ||
              (ride.booking_for === "other" ? "Guest passenger" : "Customer")}
          </p>
      ...
      <div className="rounded-lg border border-border bg-card p-4 shadow-sm">
        ...
        <div className="mt-3 grid grid-cols-[repeat(3,minmax(0,1fr))] gap-2">
          ...
          <div className="min-w-0 rounded-lg bg-muted p-2 sm:p-3">
            ...
            <p className="mt-1 truncate text-xs font-black">
              {riderLocation?.last_seen_at
                ? formatTrackingAge(riderLocation.last_seen_at)
                : "manual"}
            </p>
          </div>
        </div>
        <Button
          asChild
          className="mt-3 h-12 w-full rounded-lg bg-[#101713] text-white hover:bg-[#101713]/90"
        >
      ...
      {ride.status === "assigned" ? (
        <div className="rounded-lg border border-border bg-muted p-3">
          <Label htmlFor={`active-code-${ride.id}`}>
            Ask user for 4-digit code
          </Label>
      ...
      {ride.status === "started" ? (
        <div className="rounded-lg bg-[#101713] p-4 text-white">
          ...
          <p className="mt-1 text-xl font-black">
            Collect fare before completing
          </p>
          <p className="mt-2 text-sm leading-6 text-white/65">
            Total fare {formatMoney(ride.fare_estimate)}. Taxiro commission is{" "}
            {formatMoney(companyCommission)}; you earn{" "}
            {formatMoney(riderEarning)}.
          </p>
          <div className="mt-4 grid grid-cols-3 gap-2 text-center text-[#101713]">
            <div className="rounded-lg bg-white/90 p-2">
              <p className="text-[10px] font-bold uppercase">Fare</p>
              <p className="font-black">{formatMoney(ride.fare_estimate)}</p>
            </div>
            <div className="rounded-lg bg-white/90 p-2">
              <p className="text-[10px] font-bold uppercase">Taxiro</p>
              <p className="font-black">{formatMoney(companyCommission)}</p>
            </div>
            <div className="rounded-lg bg-secondary p-2">
              <p className="text-[10px] font-bold uppercase">You</p>
            ...
            <div className="mt-4 rounded-lg bg-white p-3 text-[#101713]">
              ...
              ride.payment_method === "driver_direct_upi" ? (
                <div className="grid gap-3 text-center">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-black/60">
                      Keep this screen open while they scan and pay{" "}
                      {formatMoney(ride.fare_estimate)}.
                    </p>
                  </div>
                  {riderProfile?.upi_qr_image_url ? (
                    <Image
                      ...
                      className="mx-auto max-h-64 w-full rounded-xl border border-border bg-white object-contain p-2"
                      height={512}
                    ...
                    <p className="rounded-xl bg-muted p-3 text-sm">
                      Upload your UPI QR from the rider menu, or collect cash
                      for this ride.
                    </p>
                  )}
                  {riderProfile?.upi_id ? (
                ...
                <p className="text-center text-sm font-semibold">
                  Collect {formatMoney(ride.fare_estimate)} in cash from the
                  customer.
                </p>
              )}
              <Button
                className="mt-4 h-14 w-full rounded-lg bg-secondary text-base font-black text-[#101713] hover:bg-secondary/90"
